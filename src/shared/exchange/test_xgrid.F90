...
program xgrid_test
  ...
  use xgrid_mod,       only : get_from_xgrid_ug, put_to_xgrid_ug, get_use_esmf_xgrid, set_x_frac_area
  use xgrid_mod,       only : esmf_wrap, esmf_wrap_put_side2_to_xgrid, esmf_wrap_get_side2_from_xgrid
  use xgrid_mod,       only : esmf_wrap_put_side1_to_xgrid, esmf_wrap_get_side1_from_xgrid
  use xgrid_mod,       only : esmf_wrap_create_xgrid, esmf_wrap_create_side1, esmf_wrap_create_side2
...
!  integer            :: num_loop = 1000
  ...
  namelist /xgrid_test_nml/ atm_input_file, atm_field_name, runoff_input_file, runoff_field_name, num_iter, &
                            ...
                            atm_nest_npes, atm_npes, lnd_npes, ice_npes, test_unstruct, num_loop
  ...
  logical              :: use_esmf_xgrid
  integer              :: nlevel, nk

  type(esmf_wrap)      :: ew_lnd, ew_ice
  type(esmf_wrap)      :: ew_x_1,ew_x_2, ew_x_3, ew_x_4
  type(esmf_wrap)      :: ew_atm,ew_atm_1,ew_atm_2,ew_atm_3
  type(esmf_wrap)      :: ew_atm_in
  ...
  ! Setup some clock stuff
  id_tot_init          = mpp_clock_id("total_init", flags=MPP_CLOCK_SYNC) 
  id_tot_run          = mpp_clock_id("total_run", flags=MPP_CLOCK_SYNC) 


  ! Time the whole init
  call mpp_clock_begin(id_tot_init)


  call xgrid_init(remap_method)
  call ensemble_manager_init() 

  use_esmf_xgrid = get_use_esmf_xgrid()
...
10   call close_file(nml_unit)
  ...
  if (pe .eq. 0) then
     ...
     write(*,*) "use_esmf_xgrid=",use_esmf_xgrid
  endif
  ...
  if( atm_input_file_exist ) then
     ...
     if(use_esmf_xgrid) then
        if(nk_lnd .NE. nk_ice) call mpp_error(FATAL, &
            "test_xgrid: nk_lnd and nk_ice must be equal when xgrid_nml use_esmf_xgrid is true")
        nlevel = nk_lnd
     else
        nlevel = 1
     endif
     allocate(x_1(nxgrid*nlevel), x_2(nxgrid*nlevel))
     allocate(x_3(nxgrid*nlevel), x_4(nxgrid*nlevel))
     x_1 = 0
     x_2 = 0
     x_3 = 0
     x_4 = 0

     ! If using ESMF wrap structures
     if (use_esmf_xgrid) then
        call esmf_wrap_create_xgrid(ew_x_1, 'LND', x_1, xmap)
        call esmf_wrap_create_xgrid(ew_x_2, 'LND', x_2, xmap)
        call esmf_wrap_create_xgrid(ew_x_3, 'LND', x_3, xmap)
        call esmf_wrap_create_xgrid(ew_x_4, 'LND', x_4, xmap)
        call esmf_wrap_create_side2(ew_lnd, lnd_data_out, 'LND', xmap)
        call esmf_wrap_create_side2(ew_ice, ice_data_out, 'OCN', xmap)
        call esmf_wrap_create_side1(ew_atm_in, atm_data_in, 'ATM', xmap)
        call esmf_wrap_create_side1(ew_atm, atm_data_out, 'ATM', xmap)
        call esmf_wrap_create_side1(ew_atm_1, atm_data_out_1, 'ATM', xmap)
        call esmf_wrap_create_side1(ew_atm_2, atm_data_out_2, 'ATM', xmap)
        call esmf_wrap_create_side1(ew_atm_3, atm_data_out_3, 'ATM', xmap)
     endif
     ...
     ! end init timing
     call mpp_clock_end(id_tot_init)

     ! time the whole run
     call mpp_clock_begin(id_tot_run)
     ...
     if (use_esmf_xgrid) then
        do n = 1, num_loop
           ...
           x_3 = 0
           x_4 = 0
          call esmf_wrap_put_side1_to_xgrid(ew_atm_in, ew_x_1, xmap) 
          call esmf_wrap_put_side1_to_xgrid(ew_atm_in, ew_x_2, xmap) 
          call esmf_wrap_put_side1_to_xgrid(ew_atm_in, ew_x_3, xmap) 
          call esmf_wrap_put_side1_to_xgrid(ew_atm_in, ew_x_4, xmap) 
        enddo
     else 
        do n = 1, num_loop
           x_1 = 0
     ...
     if (use_esmf_xgrid) then
        do n = 1, num_loop
           x_2 = 0
           call esmf_wrap_get_side2_from_xgrid(ew_lnd, ew_x_1, xmap)
           call esmf_wrap_get_side2_from_xgrid(ew_ice, ew_x_1, xmap)
           call esmf_wrap_put_side2_to_xgrid(ew_lnd, ew_x_2, xmap)
           call esmf_wrap_put_side2_to_xgrid(ew_ice, ew_x_2, xmap)
           
           call esmf_wrap_get_side1_from_xgrid(ew_atm, ew_x_2, xmap)
           call esmf_wrap_get_side1_from_xgrid(ew_atm_1, ew_x_2, xmap)
           call esmf_wrap_get_side1_from_xgrid(ew_atm_2, ew_x_2, xmap)
           call esmf_wrap_get_side1_from_xgrid(ew_atm_3, ew_x_2, xmap)
        enddo
     else
        do n = 1, num_loop
           x_2 = 0
     ...
     sum_lnd_out = 0
     do k = 1, nk_lnd
       sum_lnd_out = sum_lnd_out + mpp_global_sum(lnd_domain, lnd_area * lnd_frac(:,:,k) * lnd_data_out(:,:,k))
     enddo

     sum_ice_out = 0
     do k = 1, nk_ice
        sum_ice_out = sum_ice_out + mpp_global_sum(ice_domain, ice_area * ice_frac(:,:,k) * ice_data_out(:,:,k))
     enddo
  ...
  if(num_iter > 0) then
    if( atm_nest_npes > 0 ) call mpp_error(FATAL, &
     ...
     do n = 1, num_iter
        ...
        sum_lnd_out = 0
        do k = 1, nk_lnd
           sum_lnd_out = sum_lnd_out + mpp_global_sum(lnd_domain, & 
                                         lnd_area * lnd_frac(:,:,k) * lnd_data_out(:,:,k))
        enddo
        sum_ice_out = 0
        do k = 1, nk_ice
           sum_ice_out = sum_ice_out + mpp_global_sum(Ice_domain, &
                                          ice_area * ice_frac(:,:,k) * ice_data_out(:,:,k))
        enddo
        sum_atm_out = mpp_global_sum(atm_domain, atm_area * atm_data_out)
        write(out_unit,*) "********************** check conservation *********************** "
        write(out_unit,*) "the global area sum of atmos input data is                    : ", sum_atm_in 
