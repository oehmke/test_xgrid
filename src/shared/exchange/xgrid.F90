...
use mpp_domains_mod, only: mpp_get_compute_domain, mpp_get_compute_domains, &
                           ...
                           mpp_get_current_ntile, mpp_get_ug_compute_domain
...
use grid_mod,        only: get_grid_cell_vertices, get_grid_cell_centers, get_grid_cell_vertices          
...
!       AREA_ATM_MODEL, AREA_LND_MODEL, AREA_OCN_MODEL, &
       ...
       set_frac_area_ug, get_use_esmf_xgrid, set_x_frac_area, &
       esmf_wrap, esmf_wrap_put_side2_to_xgrid, esmf_wrap_get_side2_from_xgrid, &
       esmf_wrap_put_side1_to_xgrid, esmf_wrap_get_side1_from_xgrid, &
       esmf_wrap_create_xgrid, esmf_wrap_create_side1, esmf_wrap_create_side2
...
!  private
  ...
  real, pointer, dimension(:)     :: x_frac_area =>NULL()
  ...
  type(ESMF_XGrid)               :: esmfXGrid
end type xmap_type

! Type to wrap a stack of ESMF Fields
type esmf_wrap
  integer :: num_fields
  type(ESMF_Field), pointer, dimension(:) :: fields
  integer :: grid_index
  logical :: has_tmp_field
...
function get_use_esmf_xgrid()
   logical :: get_use_esmf_xgrid

   get_use_esmf_xgrid = use_esmf_xgrid

end function get_use_esmf_xgrid
...
!--- when monotonic_exchange is true, interp_method must be second order.
  ...
  if(use_esmf_xgrid) then
     if(trim(interp_method) .NE. 'first_order') &
        call error_mesg('xgrid_mod', 'interp_method must be first_order when use_esmf_xgrid=true', FATAL)
     write(out_unit,*)"NOTE from xgrid_mod: use ESMF xgrid for conservative exchange"
     call ESMF_Initialize (defaultCalKind=ESMF_CALKIND_GREGORIAN, &
                             defaultlogfilename="ESMF_xgrid.Log",   &
...
!  check the exchange grid file version to be used by checking the field in the file
  if(field_exist(grid_file, "AREA_ATMxOCN" ) ) then
     ...
     if(use_esmf_xgrid) then
       call error_mesg('xgrid_mod', 'use_esmf_xgrid should not be set to true when grid version = VERSION1', FATAL)
     endif 
  ...
  if(use_esmf_xgrid)  then
    !--- setup for ESMF_GRID
    call setup_esmf_grid(xmap, grid_ids, grid_domains, grid_file, atm_grid, lnd_ug_domain)
  endif


  call mpp_clock_begin(id_conservation_check)

! To turn off when checks aren't desired during timing
#if 1
  ! Need to setup x_frac_area before conservation checks
  call set_x_frac_area(xmap)
...
!    write(out_unit,* )"Rel Error: ",grid1%id,'(',xmap%grids(:)%id,')= 0.0 ',abs(xxx(1)-xxx(2))/xxx(1),abs(xxx(1)-xxx(3))/xxx(1)
!    write(out_unit,* )"ABS Error: ",grid1%id,'(',xmap%grids(:)%id,')= 0.0 ',abs(xxx(1)-xxx(2)),abs(xxx(1)-xxx(3))
!    write(out_unit,'(2E30.27)' ) abs(xxx(1)-xxx(2))/xxx(1),abs(xxx(1)-xxx(3))/xxx(1)
!    write(out_unit,'(2E30.27)') abs(xxx(1)-xxx(2)),abs(xxx(1)-xxx(3))
...
type (xmap_type),       intent(inout) :: xmap
  ...
  do g=2,size(xmap%grids(:))
    ...
    if (grid_id==grid%id) then
      ...
      grid%frac_area = f;
      if(.not. use_esmf_xgrid) call regen(xmap)
      return;
    end if
  end do
...
subroutine set_x_frac_area(xmap)
  type (xmap_type),       intent(inout) :: xmap
  integer :: nxgrid, nk, g

  if (associated(xmap%x_frac_area)) deallocate(xmap%x_frac_area)
  nxgrid = max(xmap%size,1)
  nk = size(xmap%grids(2)%frac_area,3)
  allocate(xmap%x_frac_area(nk*nxgrid))
  xmap%x_frac_area = 0.0
  do g=2,size(xmap%grids(:))
     call put_to_xgrid (xmap%grids(g)%frac_area, xmap%grids(g)%id, xmap%x_frac_area, xmap) 
  enddo

end subroutine set_x_frac_area
...
subroutine put_side1_to_xgrid(d, grid_id, x, xmap, remap_method, complete)
  real, dimension(:,:), intent(in   ), target    :: d
  character(len=3),     intent(in   )    :: grid_id
  ...
  type(ESMF_Field) :: EsmfField, XgridField  
  integer          :: rc
  real, dimension(:,:), pointer :: d_ptr=>NULL()
  real, dimension(:),   pointer :: x_ptr=>NULL()
  integer :: nxgrid, k
...
call mpp_clock_begin(id_put_1_to_xgrid_order_1)
        do k = 1, size(xmap%grids(2)%frac_area,3)
          d_ptr => d
          x_ptr(1:nxgrid) => x((k-1)*nxgrid+1:k*nxgrid)
          EsmfField = ESMF_FieldCreate(xmap%grids(1)%EsmfGrid, farrayPtr=d_ptr, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg("put_side1_to_xgrid", "fail at ESMF_FieldCreate EsmfField", FATAL)
          XgridField = ESMF_FieldCreate(xmap%EsmfXGrid, farrayPtr=x_ptr, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg("put_side1_to_xgrid", "fail at ESMF_FieldCreate XgridField", FATAL)
          call ESMF_FieldRegrid(EsmfField, XgridField, routehandle=xmap%grids(1)%g2x, &
               termorderflag=ESMF_TERMORDER_SRCSEQ, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg("put_side1_to_xgrid", "fail at ESMF_FieldRegrid", FATAL)
          call ESMF_FieldDestroy(xgridField, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg("put_side1_to_xgrid", "fail at ESMF_FieldDestroy xgridField", FATAL)
          call ESMF_FieldDestroy(EsmfField, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg("put_side1_to_xgrid", "fail at ESMF_FieldDestroy esmfField", FATAL)
        enddo
...
! </SUBROUTINE>
  ...
  subroutine point_2D_to_2D(n1,n2,data, ptr)
     integer,  intent(in) :: n1, n2
     real, intent(in), target :: data(n1,n2)
     real, pointer    :: ptr(:,:)

     ptr(1:n1,1:n2) => data

  end subroutine point_2D_to_2D
...
!BOB  Original
         ...
         do k = 1, size(xmap%grids(2)%frac_area,3)
           call point_2d_to_2d(size(d,1),size(d,2), d(:,:,k), d_ptr)
           x_ptr(1:nxgrid) => x((k-1)*nxgrid+1:k*nxgrid)

           EsmfField = ESMF_FieldCreate(xmap%grids(g)%EsmfGrid, farrayPtr=d_ptr, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid", "fail at ESMF_FieldCreate EsmfField", FATAL)
           XgridField = ESMF_FieldCreate(xmap%EsmfXGrid, farrayPtr=x_ptr, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid", "fail at ESMF_FieldCreate XgridField", FATAL)
!BOB  Try here instead
!call mpp_clock_begin(id_put_2_to_xgrid)
           call ESMF_FieldRegrid(EsmfField, XgridField, routehandle=xmap%grids(g)%g2x, &
...
!call mpp_clock_end(id_put_2_to_xgrid)
           call ESMF_FieldDestroy(xgridField, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid", "fail at ESMF_FieldDestroy xgridField", FATAL)
           call ESMF_FieldDestroy(EsmfField, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid", "fail at ESMF_FieldDestroy esmfField", FATAL)
         enddo
!BOB  Original
         call mpp_clock_end(id_put_2_to_xgrid)
       else
...
! <SUBROUTINE NAME="esmf_wrap_create_side1" INTERFACE="put_to_xgrid">
!   <IN NAME="d"  TYPE="real" DIM="(:,:,:)" > </IN>
!   <IN NAME="grid_id"  TYPE=" character(len=3)"  > </IN>
!   <INOUT NAME="x"  TYPE="real" DIM="(:)" > </INOUT>
!   <INOUT NAME="xmap"  TYPE="xmap_type"  > </INOUT>
subroutine esmf_wrap_create_side1(ew, d, grid_id, xmap)
type(esmf_wrap) :: ew
real, dimension(:,:), intent(in   ), target :: d
character(len=3),       intent(in   ) :: grid_id
type (xmap_type),       intent(inout) :: xmap
  ...
  real, dimension(:,:), pointer :: d_ptr=>NULL()
  real, dimension(:),   pointer :: x_ptr=>NULL()
  integer :: g, k, size, grid_index, num_levels
  ...
  ! Get size
  num_levels=1 ! Side 1 only has one level
  
  ! Allocate struct
  ew%grid_index=grid_index
  ew%num_fields=num_levels
  allocate(ew%fields(ew%num_fields))

  ! Create Field
  d_ptr=>d
  ew%fields(1) = ESMF_FieldCreate(xmap%grids(grid_index)%EsmfGrid, &
       farrayPtr=d_ptr, rc=rc)
  if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid", "fail at ESMF_FieldCreate EsmfField", FATAL)

  ! Does not have a tmp field
  ew%has_tmp_field=.false.

end subroutine esmf_wrap_create_side1
! </SUBROUTINE>

!#######################################################################

! <SUBROUTINE NAME="esmf_wrap_create_side2" INTERFACE="put_to_xgrid">
!   <IN NAME="d"  TYPE="real" DIM="(:,:,:)" > </IN>
!   <IN NAME="grid_id"  TYPE=" character(len=3)"  > </IN>
!   <INOUT NAME="x"  TYPE="real" DIM="(:)" > </INOUT>
!   <INOUT NAME="xmap"  TYPE="xmap_type"  > </INOUT>
subroutine esmf_wrap_create_side2(ew, d, grid_id, xmap)
type(esmf_wrap) :: ew
real, dimension(:,:,:), intent(in   ), target :: d
character(len=3),       intent(in   ) :: grid_id
type (xmap_type),       intent(inout) :: xmap
  ...
  ! Get size
  num_levels=size(xmap%grids(grid_index)%frac_area,3)
  ...
  do k = 1, num_levels
     call point_2d_to_2d(size(d,1),size(d,2), d(:,:,k), d_ptr)
     ew%fields(k) = ESMF_FieldCreate(xmap%grids(grid_index)%EsmfGrid, farrayPtr=d_ptr, rc=rc)
     if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid", "fail at ESMF_FieldCreate EsmfField", FATAL)
  end do

  ! Does not have a tmp field
  ew%has_tmp_field=.true.

end subroutine esmf_wrap_create_side2
! </SUBROUTINE>


!#######################################################################

! <SUBROUTINE NAME="esmf_wrap_create_xgrid" INTERFACE="put_to_xgrid">
!   <IN NAME="d"  TYPE="real" DIM="(:,:,:)" > </IN>
!   <IN NAME="grid_id"  TYPE=" character(len=3)"  > </IN>
!   <INOUT NAME="x"  TYPE="real" DIM="(:)" > </INOUT>
!   <INOUT NAME="xmap"  TYPE="xmap_type"  > </INOUT>
subroutine esmf_wrap_create_xgrid(ew, grid_id, x, xmap)
type(esmf_wrap) :: ew
character(len=3),       intent(in   ) :: grid_id
real, dimension(:),     intent(inout), target :: x
type (xmap_type),       intent(inout) :: xmap
  ...
  real, dimension(:,:), pointer :: d_ptr=>NULL()
  real, dimension(:),   pointer :: x_ptr=>NULL()
  integer :: g, k, size, grid_index, num_levels, nxgrid
  type(ESMF_Typekind_Flag) :: field_tk
  ...
  do k = 1, num_levels
     x_ptr(1:nxgrid) => x((k-1)*nxgrid+1:k*nxgrid)
     ew%fields(k) = ESMF_FieldCreate(xmap%EsmfXGrid, farrayPtr=x_ptr, rc=rc)
     if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid", "fail at ESMF_FieldCreate XgridField", FATAL)
   end do

  ! Also has a tmp field
  ew%has_tmp_field=.true.

  ! Create a temporary field with the same typekind as other fields
  call ESMF_FieldGet(ew%fields(1), typekind=field_tk, rc=rc)
  if(rc /= ESMF_SUCCESS) call error_mesg("esmf_wrap_create_side1", "fail at ESMF_FieldGet()", FATAL)

  ew%tmp_field = ESMF_FieldCreate(xmap%EsmfXGrid, &
       typekind=field_tk, rc=rc)
  if(rc /= ESMF_SUCCESS) call error_mesg("esmf_wrap_create_side1", "fail at ESMF_FieldCreate EsmfField", FATAL)


end subroutine esmf_wrap_create_xgrid
! </SUBROUTINE>

!#######################################################################

! <SUBROUTINE NAME="esmf_wrap_put_side2_to_xgrid" INTERFACE="put_to_xgrid">
!   <IN NAME="d"  TYPE="real" DIM="(:,:,:)" > </IN>
!   <IN NAME="grid_id"  TYPE=" character(len=3)"  > </IN>
!   <INOUT NAME="x"  TYPE="real" DIM="(:)" > </INOUT>
!   <INOUT NAME="xmap"  TYPE="xmap_type"  > </INOUT>
subroutine esmf_wrap_put_side1_to_xgrid(ew_s1, ew_x, xmap)
  type(esmf_wrap) :: ew_s1,ew_x
  type (xmap_type),       intent(inout) :: xmap
  integer          :: rc
  integer :: k
...
end subroutine esmf_wrap_put_side1_to_xgrid
! </SUBROUTINE>
...
!   <INOUT NAME="x"  TYPE="real" DIM="(:)" > </INOUT>
!   <INOUT NAME="xmap"  TYPE="xmap_type"  > </INOUT>
subroutine esmf_wrap_put_side2_to_xgrid(ew_s2, ew_x, xmap)
  type(esmf_wrap) :: ew_s2,ew_x
  type (xmap_type),       intent(inout) :: xmap
  integer          :: rc
  integer :: k

  ! Error checks
  if (ew_s2%num_fields .ne. ew_x%num_fields) &
    call error_mesg ('xgrid_mod',  &
                     'put_to_xgrid expects the number of fields to match', FATAL)

  ! Start timer
  call mpp_clock_begin(id_put_2_to_xgrid)

  ! Loop regridding fields
  do k = 1, ew_s2%num_fields
     call ESMF_FieldRegrid(ew_s2%fields(k), ew_x%fields(k), & 
          routehandle=xmap%grids(ew_s2%grid_index)%g2x, &
          termorderflag=ESMF_TERMORDER_SRCSEQ, zeroregion=ESMF_REGION_EMPTY, rc=rc)
     if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid", "fail at ESMF_FieldRegrid", FATAL)
  enddo
...
end subroutine esmf_wrap_put_side2_to_xgrid
! </SUBROUTINE>


!#######################################################################

! <SUBROUTINE NAME="esmf_wrap_get_side1_from_xgrid" INTERFACE="put_to_xgrid">
!   <IN NAME="d"  TYPE="real" DIM="(:,:,:)" > </IN>
!   <IN NAME="grid_id"  TYPE=" character(len=3)"  > </IN>
!   <INOUT NAME="x"  TYPE="real" DIM="(:)" > </INOUT>
!   <INOUT NAME="xmap"  TYPE="xmap_type"  > </INOUT>
subroutine esmf_wrap_get_side1_from_xgrid(ew_s1, ew_x, xmap)
  type(esmf_wrap) :: ew_s1,ew_x
  type (xmap_type),  intent(inout) :: xmap
  integer          :: rc
  integer :: nxgrid, i,k
  real(ESMF_KIND_R8), dimension(:), pointer :: x_tmp=>NULL()
  real(ESMF_KIND_R8), dimension(:), pointer :: x_level=>NULL()

  ! Start timer
  call mpp_clock_begin(id_get_1_from_xgrid)

  ! Get temporary
  if (.not. ew_x%has_tmp_field) then
     if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid", "fail because there isn't a tmp Field", FATAL)
  endif
  ...
  ! Get pointer from tmp Field
  call ESMF_FieldGet(ew_x%tmp_field, farrayPtr=x_tmp, rc=rc)
  if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid", "fail at ESMF_FieldGet()", FATAL)

  ! Init tmp
  x_tmp=0.0

  ! Loop reducing fields
  do k = 1, ew_x%num_fields

     ! Get pointer from xgrid level Field
     call ESMF_FieldGet(ew_x%fields(k), farrayPtr=x_level, rc=rc)
     if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid", "fail at ESMF_FieldGet()", FATAL)

     ! Sum with fraction into temporary field
     do i = 1, nxgrid
        x_tmp(i)=x_tmp(i) + x_level(i)*xmap%x_frac_area((k-1)*nxgrid+i)
     enddo
  enddo

  ! Regrid temp field to side 1 field
  call ESMF_FieldRegrid(ew_x%tmp_field, ew_s1%fields(1), &
       routehandle=xmap%grids(ew_s1%grid_index)%x2g, &
       termorderflag=ESMF_TERMORDER_SRCSEQ, rc=rc)
  if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid", "fail at ESMF_FieldRegrid", FATAL)

  ! Stop timer
  call mpp_clock_end(id_get_1_from_xgrid)

end subroutine esmf_wrap_get_side1_from_xgrid
! </SUBROUTINE>

!#######################################################################

! <SUBROUTINE NAME="esmf_wrap_get_side2_from_xgrid" INTERFACE="put_to_xgrid">
!   <IN NAME="d"  TYPE="real" DIM="(:,:,:)" > </IN>
!   <IN NAME="grid_id"  TYPE=" character(len=3)"  > </IN>
!   <INOUT NAME="x"  TYPE="real" DIM="(:)" > </INOUT>
!   <INOUT NAME="xmap"  TYPE="xmap_type"  > </INOUT>
subroutine esmf_wrap_get_side2_from_xgrid(ew_s2, ew_x, xmap)
  type(esmf_wrap) :: ew_s2,ew_x
  type (xmap_type),       intent(inout) :: xmap
  integer          :: rc
  integer :: i,j,k
  ...
  if (ew_s2%num_fields .ne. ew_x%num_fields) &
    call error_mesg ('xgrid_mod',  &
                     'get_from_xgrid expects the number of fields to match', FATAL)
  ...
  ! Loop regridding fields
  do k = 1, ew_s2%num_fields
     call ESMF_FieldRegrid(ew_x%fields(k), ew_s2%fields(k), &
          routehandle=xmap%grids(ew_s2%grid_index)%x2g, &
          termorderflag=ESMF_TERMORDER_SRCSEQ, rc=rc)
     if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid", "fail at ESMF_FieldRegrid", FATAL)

     ! Here you could loop through and zero out entries in ew_s2%fields(k) where the corresponding
     ! frac_area location is 0.0. This would solve issues where the side2 field data is used without
     ! the application of frac_area. Since gfdl doesn't store data where the frac_area is 0.0, but
     ! ESMF does. This could possibly lead to a mismatch. However, since such cases seem like they would
     ! be unlikely, I don't want to spend the extra processor time to do it, but if it needs to be done
     ! this would probably be the place to do it. 
  enddo
...
end subroutine esmf_wrap_get_side2_from_xgrid
! </SUBROUTINE>
...
subroutine get_side1_from_xgrid(d, grid_id, x, xmap, complete)
  ...
  real, dimension(:),   pointer :: x_ptr=>NULL()
  integer :: nxgrid, k, i
  real, target    :: xtmp(max(Xmap%size, 1))
  ...
  call mpp_clock_begin(id_get_1_from_xgrid)
        ...
        do k = 1, size(xmap%grids(2)%frac_area,3)
           do i = 1, nxgrid
              xtmp(i) = xtmp(i) + x((k-1)*nxgrid+i)*xmap%x_frac_area((k-1)*nxgrid+i)
           enddo
        enddo
        x_ptr => xtmp
        EsmfField = ESMF_FieldCreate(xmap%grids(1)%EsmfGrid, farrayPtr=d_ptr, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid", "fail at ESMF_FieldCreate EsmfField", FATAL)
        XgridField = ESMF_FieldCreate(xmap%EsmfXGrid, farrayPtr=x_ptr, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid", "fail at ESMF_FieldCreate XgridField", FATAL)
        call ESMF_FieldRegrid(XgridField, EsmfField, routehandle=xmap%grids(1)%x2g, &
             termorderflag=ESMF_TERMORDER_SRCSEQ, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid", "fail at ESMF_FieldRegrid", FATAL)
        call ESMF_FieldDestroy(xgridField, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid", "fail at ESMF_FieldDestroy xgridField", FATAL)
        call ESMF_FieldDestroy(EsmfField, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid", "fail at ESMF_FieldDestroy esmfField", FATAL)
  call mpp_clock_end(id_get_1_from_xgrid)
...
! BOB 
  call mpp_clock_begin(id_get_2_from_xgrid)
          do k = 1, size(xmap%grids(2)%frac_area,3)
           ...
           EsmfField = ESMF_FieldCreate(xmap%grids(g)%EsmfGrid, farrayPtr=d_ptr, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("get_side2_from_xgrid", "fail at ESMF_FieldCreate EsmfField", FATAL)
           XgridField = ESMF_FieldCreate(xmap%EsmfXGrid, farrayPtr=x_ptr, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("get_side2_from_xgrid", "fail at ESMF_FieldCreate XgridField", FATAL)
! BOB Try here instead
! call mpp_clock_begin(id_get_2_from_xgrid)
           call ESMF_FieldRegrid(XgridField, EsmfField, routehandle=xmap%grids(g)%x2g, &
                termorderflag=ESMF_TERMORDER_SRCSEQ, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("get_side2_from_xgrid", "fail at ESMF_FieldRegrid", FATAL)
! BOB Try here instead
! call mpp_clock_end(id_get_2_from_xgrid)
           call ESMF_FieldDestroy(xgridField, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("get_side2_from_xgrid", "fail at ESMF_FieldDestroy xgridField", FATAL)
           call ESMF_FieldDestroy(EsmfField, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("get_side2_from_xgrid", "fail at ESMF_FieldDestroy esmfField", FATAL)
          enddo
! BOB  
 call mpp_clock_end(id_get_2_from_xgrid)
        else
...
subroutine get_side1_from_xgrid_ug(d, grid_id, x, xmap, complete)
  ...
  if (grid_id==xmap%grids(1)%id) then
     if(use_esmf_xgrid ) then
        ...
        x_ptr => x
        EsmfField = ESMF_FieldCreate(xmap%grids(1)%EsmfGrid, farrayPtr=d_ptr, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid_ug", "fail at ESMF_FieldCreate EsmfField", FATAL)
        XgridField = ESMF_FieldCreate(xmap%EsmfXGrid, farrayPtr=x_ptr, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid_ug", "fail at ESMF_FieldCreate XgridField", FATAL)
        call ESMF_FieldRegrid(XgridField, EsmfField, routehandle=xmap%grids(1)%x2g, &
             termorderflag=ESMF_TERMORDER_SRCSEQ, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid_ug", "fail at ESMF_FieldRegrid", FATAL)
        call ESMF_FieldDestroy(xgridField, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid_ug", "fail at ESMF_FieldDestroy xgridField", FATAL)
        call ESMF_FieldDestroy(EsmfField, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("get_side1_from_xgrid_ug", "fail at ESMF_FieldDestroy esmfField", FATAL)
        return
     endif
...
subroutine put_side1_to_xgrid_ug(d, grid_id, x, xmap, complete)
  ...
  if (grid_id==xmap%grids(1)%id) then
     if( use_esmf_xgrid ) then
        d_ptr => d
        x_ptr => x
        EsmfField = ESMF_FieldCreate(xmap%grids(1)%EsmfMesh, farrayPtr=d_ptr, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("put_side1_to_xgrid_ug", "fail at ESMF_FieldCreate EsmfField", FATAL)
        XgridField = ESMF_FieldCreate(xmap%EsmfXGrid, farrayPtr=x_ptr, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("put_side1_to_xgrid_ug", "fail at ESMF_FieldCreate XgridField", FATAL)
        call ESMF_FieldRegrid(EsmfField, XgridField, routehandle=xmap%grids(1)%g2x, &
             termorderflag=ESMF_TERMORDER_SRCSEQ, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("put_side1_to_xgrid_ug", "fail at ESMF_FieldRegrid", FATAL)
        call ESMF_FieldDestroy(xgridField, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("put_side1_to_xgrid_ug", "fail at ESMF_FieldDestroy xgridField", FATAL)
        call ESMF_FieldDestroy(EsmfField, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("put_side1_to_xgrid_ug", "fail at ESMF_FieldDestroy esmfField", FATAL)
        return
     endif
  ...
  do g=2,size(xmap%grids(:))
     if (grid_id==xmap%grids(g)%id) then
        if(use_esmf_xgrid ) then
           ...
           x_ptr => x
           EsmfField = ESMF_FieldCreate(xmap%grids(g)%EsmfGrid, farrayPtr=d_ptr, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid_ug", "fail at ESMF_FieldCreate EsmfField", FATAL)
           XgridField = ESMF_FieldCreate(xmap%EsmfXGrid, farrayPtr=x_ptr, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid_ug", "fail at ESMF_FieldCreate XgridField", FATAL)
           call ESMF_FieldRegrid(EsmfField, XgridField, routehandle=xmap%grids(g)%g2x, &
                termorderflag=ESMF_TERMORDER_SRCSEQ, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid_ug", "fail at ESMF_FieldRegrid", FATAL)
           call ESMF_FieldDestroy(xgridField, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid_ug", "fail at ESMF_FieldDestroy xgridField", FATAL)
           call ESMF_FieldDestroy(EsmfField, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("put_side2_to_xgrid_ug", "fail at ESMF_FieldDestroy esmfField", FATAL)
        else
  ...
  do g=2,size(xmap%grids(:))
     if (grid_id==xmap%grids(g)%id) then
        if(use_esmf_xgrid ) then
           ...
           x_ptr => x
           EsmfField = ESMF_FieldCreate(xmap%grids(g)%EsmfGrid, farrayPtr=d_ptr, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("get_side2_from_xgrid_ug", "fail at ESMF_FieldCreate EsmfField", FATAL)
           XgridField = ESMF_FieldCreate(xmap%EsmfXGrid, farrayPtr=x_ptr, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("get_side2_from_xgrid_ug", "fail at ESMF_FieldCreate XgridField", FATAL)
           call ESMF_FieldRegrid(XgridField, EsmfField, routehandle=xmap%grids(g)%x2g, &
                termorderflag=ESMF_TERMORDER_SRCSEQ, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("get_side2_from_xgrid_ug", "fail at ESMF_FieldRegrid", FATAL)
           call ESMF_FieldDestroy(xgridField, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("get_side2_from_xgrid_ug", "fail at ESMF_FieldDestroy xgridField", FATAL)
           call ESMF_FieldDestroy(EsmfField, rc=rc)
           if(rc /= ESMF_SUCCESS) call error_mesg("get_side2_from_xgrid_ug", "fail at ESMF_FieldDestroy esmfField", FATAL)
        else
...
subroutine setup_esmf_grid(xmap, grid_ids, grid_domains, grid_file, atm_grid, lnd_ug_domain)
  type (xmap_type),             intent(inout) :: xmap
  character(len=3), dimension(:), intent(in ) :: grid_ids
  type(Domain2d), dimension(:),   intent(in ) :: grid_domains
  character(len=*),               intent(in ) :: grid_file
  ...
  integer :: tile_ids(1), rc
  integer :: lnd_ug_id, g, sg_face, ls, le, l, n
  integer :: isc, iec, jsc, jec, nxc, nyc
  type(grid_type), pointer :: grid=>NULL()
  type(ESMF_Mesh)     :: xmesh
  type(ESMF_DELayout) :: delayout
  type(ESMF_DistGrid) :: distgrid
  type(ESMF_Field)    :: esmfField(size(grid_ids(:)))
  type(ESMF_Field)    :: esmfXgridField
  real(ESMF_KIND_R8), pointer :: dataPtr_xcen(:,:), dataPtr_ycen(:,:)
  real(ESMF_KIND_R8), pointer :: dataPtr_xcen1(:,:), dataPtr_ycen1(:,:)
  real(ESMF_KIND_R8), pointer :: dataPtr_xcor(:,:), dataPtr_ycor(:,:)
  integer(ESMF_KIND_I4), pointer :: dataPtr_mask(:,:)
  real(ESMF_KIND_R8), pointer :: dataPtr_frac(:,:)
  real(ESMF_KIND_R8), pointer :: dataPtr_area(:,:)
  real, allocatable :: elemCornerCoords3(:,:,:)
  real,    allocatable :: lonb(:,:), latb(:,:)
  real,    allocatable :: xcor(:,:), ycor(:,:)
  real,    allocatable :: ocn_mask(:,:)
  integer, allocatable :: xb(:), yb(:), xe(:), ye(:)
  integer, allocatable :: deBlockList(:,:,:), deLabelList(:), petMap(:)
  character(len=32) :: ocean_mask_file = 'INPUT/ocean_mask.nc'

  type(ESMF_VM)    :: vm
  integer          :: lpet
  type(ESMF_DistGrid) :: cornerDG
  type(ESMF_Array)    :: cornerXArray, cornerYArray
  type(ESMF_Grid)     :: regDecompGrid
  ...
  type(ESMF_Field)  :: src_frac_x2g
  type(ESMF_Field)  :: src_frac_g2x
  type(ESMF_Field)  :: dst_frac_g2x
  type(ESMF_Array)  :: tmpArray, maskArray
  type(ESMF_Distgrid) :: tmpDistgrid

  type(ESMF_Field)  :: dst_merge_frac_x2g(size(grid_ids(:)))
  type(ESMF_Field)  :: dst_frac_x2g(size(grid_ids(:)))
  real(ESMF_KIND_R8) :: min_frac,max_frac

  type(ESMF_Field)  :: tst_fields(size(grid_ids(:)))
  type(ESMF_Field)  :: tst_field_XGrid
  type(ESMF_Field)  :: tst_field_XGrid2
  integer :: i1,i2
  real(ESMF_KIND_R8) :: l_sum(2),g_sum(2)
  real(ESMF_KIND_R8) :: atm_sum, xgrid_sum,lo_sum, datm_sum
  real(ESMF_KIND_R8) :: atm_area, xgrid_area,lo_area, datm_area
  real(ESMF_KIND_R8) :: atm_norm_sum, xgrid_norm_sum,lo_norm_sum,datm_norm_sum
  integer :: localPet

  real(ESMF_KIND_R8), pointer :: dataPtr_tst(:,:)
  real(ESMF_KIND_R8), pointer :: dataPtr_merge_frac(:,:)
  real(ESMF_KIND_R8), pointer :: dataPtr_tst_Xgrid(:)
  real(ESMF_KIND_R8), pointer :: dataPtr_tst_Xgrid2(:)
  real(ESMF_KIND_R8), pointer :: dataPtr_area_Xgrid(:)
  real(ESMF_KIND_R8), pointer :: dataPtr_frac_Xgrid(:)
  integer(ESMF_KIND_I4), pointer :: dataPtr_I4(:,:)
  real(ESMF_KIND_R8), pointer :: dataPtr_R8(:,:)
  integer, pointer :: seqInd(:)
  real(ESMF_KIND_R8), parameter :: sph_area=12.56637061435917
 ...
 if (localPet .eq. 0) write(*,*) "BOB_XGRID_DEBUG DEFINED"
#endif
  ...
  if(present(lnd_ug_domain)) then
   ...
   do g = 1, size(grid_ids(:))
      grid => xmap%grids(g)
      call mpp_get_global_domain(grid_domains(g), xsize=grid%ni, ysize=grid%nj)
      tile_ids = mpp_get_tile_id(grid_domains(g))
      sg_face = tile_ids(1)
      if(g==lnd_ug_id) then ! unstructured grid for land
         if( mpp_get_current_ntile(grid_domains(g)) > 1) call error_mesg('xgrid_mod', &
              'number of tiles on current PE is greater than 1', FATAL)

         call mpp_get_UG_compute_domain(lnd_ug_domain, ls, le)
         allocate(lonb(ls:le,4), latb(ls:le,4))
         call get_grid_cell_vertices('LND', sg_face, lonb, latb, grid_domains(g), lnd_ug_domain)
         ! Allocate and fill element corner coordinate array.
         allocate(elemCornerCoords3(2,4,ls:le))
         do l = ls, le
            elemCornerCoords3(1,:,l) = lonb(l,:)
            elemCornerCoords3(2,:,l) = latb(l,:)
         enddo

         grid%esmfMesh=ESMF_MeshCreate(parametricDim=2, &
              coordSys=ESMF_COORDSYS_CART,   &
              elementType=ESMF_MESHELEMTYPE_QUAD, &
              elementCornerCoords=elemCornerCoords3, &
              rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_MeshCreate', FATAL)

         esmfField(g)=ESMF_FieldCreate(grid%esmfMesh,typekind=ESMF_TYPEKIND_R8, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldCreate for grid%esmfMesh', FATAL)
      else 
        if( grid_ids(g) == 'OCN' ) then
          allocate(xb(grid%npes), yb(grid%npes), xe(grid%npes), ye(grid%npes))
          call mpp_get_compute_domains(grid_domains(g), xbegin=xb, xend=xe, ybegin=yb, yend=ye)
          allocate(deBlockList(2,2,grid%npes))
          allocate(petMap(grid%npes))
          allocate(deLabelList(grid%npes))
          do n = 1, grid%npes
            deLabelList(n) = n
            ...
            petMap(n) = grid%pelist(n-1)
            !write(stdout(),*) 'n=', n, deBlockList(:,:,n)
          enddo

          ! Create connectionList describing tripole
          ! periodicity
          call ESMF_DistgridConnectionSet(connection=connectionList(1), &
               tileIndexA=1,tileIndexB=1, &
               positionVector=(/grid%ni,0/), &
               orientationVector=(/1,2/), rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_DistgridConnectionSet', FATAL)


          call ESMF_DistgridConnectionSet(connection=connectionList(2), &
               tileIndexA=1,tileIndexB=1, &
               positionVector=(/grid%ni+1,2*grid%nj+1/), &
               orientationVector=(/-1,-2/), rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_DistgridConnectionSet', FATAL)
          ...
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_DELayoutCreate', FATAL)
          distgrid = ESMF_DistGridCreate(minIndex=(/1,1/), maxIndex=(/grid%ni, grid%nj/), &
               indexflag=ESMF_INDEX_GLOBAL, &
               ! NOT NECESSARY RIGHT NOW  connectionList=connectionList,                 &
               deBlockList=deBlockList, &
               delayout=delayout, &
               rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_DistGridCreate', FATAL)
          deallocate(xb, yb, xe, ye)
          deallocate(deBlockList, petMap, deLabelList)

          grid%esmfGrid = ESMF_GridCreate(distgrid=distgrid, &
                                          GridEdgeLWidth=(/0,0/), GridEdgeUWidth=(/1,1/), &
                                          coordSys = ESMF_COORDSYS_SPH_DEG, &
                                          indexflag=ESMF_INDEX_GLOBAL, &
                                          rc = rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_GridCreate', FATAL)

          call ESMF_GridAddCoord(grid%esmfgrid, staggerLoc=ESMF_STAGGERLOC_CENTER, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_GridAddCoord ESMF_STAGGERLOC_CENTER', FATAL)
          call ESMF_GridAddCoord(grid%esmfgrid, staggerLoc=ESMF_STAGGERLOC_CORNER, &
               rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_GridAddCoord ESMF_STAGGERLOC_CORNER', FATAL)

          call ESMF_GridAddItem(grid%esmfgrid, itemFlag=ESMF_GRIDITEM_MASK, itemTypeKind=ESMF_TYPEKIND_I4, &
                                staggerLoc=ESMF_STAGGERLOC_CENTER, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_GridAddItem ESMF_GRIDITEM_MASK', FATAL)

          call ESMF_GridGetCoord(grid%esmfgrid, coordDim=1, &
                                 staggerloc=ESMF_STAGGERLOC_CENTER, &
                                 farrayPtr=dataPtr_xcen, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_GridGetCoord dataPtr_xcen', FATAL)
          call ESMF_GridGetCoord(grid%esmfgrid, coordDim=2, &
                                 staggerloc=ESMF_STAGGERLOC_CENTER, &
                                 farrayPtr=dataPtr_ycen, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_GridGetCoord dataPtr_ycen', FATAL)
          call ESMF_GridGetCoord(grid%esmfgrid, coordDim=1, &
                                 staggerloc=ESMF_STAGGERLOC_CORNER, &
                                 farrayPtr=dataPtr_xcor, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_GridGetCoord dataPtr_xcor', FATAL)
          call ESMF_GridGetCoord(grid%esmfgrid, coordDim=2, &
                                 staggerloc=ESMF_STAGGERLOC_CORNER, &
                                 farrayPtr=dataPtr_ycor, rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_GridGetCoord dataPtr_ycor', FATAL)

          call get_grid_cell_centers(grid_ids(g), sg_face, dataPtr_xcen, dataPtr_ycen, grid_domains(g))
          call mpp_get_compute_domain(grid_domains(g), xsize=nxc, ysize=nyc)
          allocate(xcor(nxc+1,nyc+1), ycor(nxc+1,nyc+1))
          call get_grid_cell_vertices(grid_ids(g), sg_face, xcor, ycor, grid_domains(g))
          dataPtr_xcor = xcor(1:size(dataPtr_xcor,1), 1:size(dataPtr_xcor,2))
          dataPtr_ycor = ycor(1:size(dataPtr_ycor,1), 1:size(dataPtr_ycor,2))

          deallocate(xcor, ycor)
          !--set mask for ocean grid.
           call mpp_get_compute_domain(grid_domains(g), isc, iec, jsc, jec)
           allocate(ocn_mask(isc:iec,jsc:jec))
           call read_data(ocean_mask_file, 'mask', ocn_mask, domain=grid_domains(g))

           call ESMF_GridGetItem(grid%esmfgrid, itemflag=ESMF_GRIDITEM_MASK, &
                                 staggerloc=ESMF_STAGGERLOC_CENTER, &
                                 farrayPtr=dataPtr_mask, rc=rc)
! print*, '#####', mpp_pe(), size(dataPtr_mask,1), size(dataPtr_mask,2), size(ocn_mask,1), size(ocn_mask,2)
          ...
          !write(stdout(),*) g, 'Grid ID: '//trim(grid_ids(g))
          if( grid_ids(g) == 'ATM' ) then
            grid%esmfGrid=ESMF_GridCreateMosaic(filename=trim("INPUT/atmos_mosaic.nc"), &
              tileFilePath="./INPUT/", regDecompPTile=decomptile, &
              staggerLocList=(/ESMF_STAGGERLOC_CENTER, ESMF_STAGGERLOC_CORNER/), &
              indexflag=ESMF_INDEX_GLOBAL, &
              rc=rc)
            if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at atmos grid create', FATAL)
          else
            grid%esmfGrid=ESMF_GridCreateMosaic(filename=trim("INPUT/land_mosaic.nc"), &
              tileFilePath="./INPUT/", regDecompPTile=decomptile, &
              staggerLocList=(/ESMF_STAGGERLOC_CENTER, ESMF_STAGGERLOC_CORNER/), &
              indexflag=ESMF_INDEX_GLOBAL, &
              rc=rc)
            if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at land grid create', FATAL)
          endif

          ! Repopulate coordinates so that it contains the model specific values
          call ESMF_GridAddCoord(grid%esmfgrid, staggerLoc=ESMF_STAGGERLOC_CORNER, &
               rc=rc)
          if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_GridAddCoord ESMF_STAGGERLOC_CORNER', FATAL)
          call ESMF_GridGetCoord(grid%esmfgrid, coordDim=1, &
         ...
         esmfField(g)=ESMF_FieldCreate(grid%esmfGrid,typekind=ESMF_TYPEKIND_R8, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldCreate for grid%esmfGrid', FATAL)
      end if
   end do

   !--- setup ESMF_XGRID
   if( size(grid_ids(:)) == 2 ) then
      if(lnd_ug_id > 0) then
         if(lnd_ug_id .NE. 1) call error_mesg('xgrid_mod', &
                               'when size(grids_id(:))==2 and land_ug_id >0, land_ug_id must be 1', FATAL)
         xmap%esmfXgrid = ESMF_XGridCreate(sideAMesh=(/xmap%grids(1)%esmfMesh/), sideBGrid=(/xmap%grids(2)%esmfGrid/), &
                        sideBMaskValues=(/0/), storeOverlay=.true., rc=rc)
      else
         xmap%esmfXgrid = ESMF_XGridCreate(sideAGrid=(/xmap%grids(1)%esmfGrid/), sideBGrid=(/xmap%grids(2)%esmfGrid/), &
                        sideBMaskValues=(/0/), storeOverlay=.true., rc=rc)
      endif
   else if( size(grid_ids(:)) == 3 ) then
      if(lnd_ug_id > 0) then
         if(lnd_ug_id .NE. 2) call error_mesg('xgrid_mod', &
                               'when size(grids_id(:))==3 and land_ug_id >0, land_ug_id must be 2', FATAL)
         xmap%esmfXgrid = ESMF_XGridCreate(sideAGrid=(/xmap%grids(1)%esmfGrid/), sideBMesh=(/xmap%grids(2)%esmfMesh/), &
                        sideBGrid=(/xmap%grids(3)%esmfGrid/), &
                        sideBMaskValues=(/0/), storeOverlay=.true., rc=rc)
      else

       ! Write out grids for debugging
#if 0
        call ESMF_GridWriteVTK(xmap%grids(1)%esmfGrid, filename="grid1", rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at grid1 writevtk', FATAL)
        call ESMF_GridWriteVTK(xmap%grids(1)%esmfGrid, filename="grid1cor", staggerloc=ESMF_STAGGERLOC_CORNER, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at grid1 corner writevtk', FATAL)

        call ESMF_GridGetItem(xmap%grids(2)%esmfGrid, itemflag=ESMF_GRIDITEM_MASK, &
             staggerloc=ESMF_STAGGERLOC_CENTER, array=maskArray, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at grid2 get mask', FATAL)
        call ESMF_ArrayGet(maskArray, distgrid=tmpDistgrid, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ArrayGet', FATAL)
    
        ! Store mask data for grid 2
        tmpArray=ESMF_ArrayCreate(tmpDistgrid, ESMF_TYPEKIND_R8, indexflag=ESMF_INDEX_GLOBAL, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ArrayGet', FATAL)

        call ESMF_ArrayGet(maskArray, farrayPtr=dataPtr_I4, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ArrayGet', FATAL)

        call ESMF_ArrayGet(tmpArray, farrayPtr=dataPtr_R8, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ArrayGet', FATAL)
#if 0
        write(*,*) "data_PtrI4(1)=",lbound(dataPtr_I4,1), ubound(dataPtr_I4,1)
        write(*,*) "data_PtrI4(2)=",lbound(dataPtr_I4,2), ubound(dataPtr_I4,2)

        write(*,*) "data_PtrR8(1)=",lbound(dataPtr_R8,1), ubound(dataPtr_R8,1)
        write(*,*) "data_PtrR8(2)=",lbound(dataPtr_R8,2), ubound(dataPtr_R8,2)
#endif    

        do i1=lbound(dataPtr_I4,1), ubound(dataPtr_I4,1)
           do i2=lbound(dataPtr_I4,2), ubound(dataPtr_I4,2)
              dataPtr_R8(i1,i2) = REAL(dataPtr_I4(i1,i2),ESMF_KIND_R8)
           enddo
        enddo

        call ESMF_GridWriteVTK(xmap%grids(2)%esmfGrid, filename="grid2", array1=tmpArray, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at grid2 writevtk', FATAL)
        call ESMF_ArrayDestroy(tmpArray, rc=rc) 
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ArrayDestroy', FATAL)


        call ESMF_GridWriteVTK(xmap%grids(2)%esmfGrid, filename="grid2cor", staggerloc=ESMF_STAGGERLOC_CORNER, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at grid2 corner writevtk', FATAL)
        call ESMF_GridWriteVTK(xmap%grids(3)%esmfGrid, filename="grid3", rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at grid3 writevtk', FATAL)
        call ESMF_GridWriteVTK(xmap%grids(3)%esmfGrid, filename="grid3cor", staggerloc=ESMF_STAGGERLOC_CORNER, rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at grid3 corner writevtk', FATAL)
        call ESMF_GridCellWriteVTK(xmap%grids(3)%esmfGrid, filename="grid3cell", rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at grid3 corner writevtk', FATAL)
#endif

        xmap%esmfXgrid = ESMF_XGridCreate(sideAGrid=(/xmap%grids(1)%esmfGrid/), &
                        sideBGrid=(/xmap%grids(2)%esmfGrid, xmap%grids(3)%esmfGrid/), &
                        sideBMaskValues=(/1/), storeOverlay=.true., rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at creating XGrid', FATAL)

      endif
   else
      call error_mesg('xgrid_mod', 'size(grids_id(:)) must be 2 or 3', FATAL)
   end if

   call ESMF_XGridGet(xmap%esmfXgrid, mesh=xmesh, rc=rc)
   if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at getting mesh from XGrid', FATAL)

   call ESMF_MeshGet(xmesh, numOwnedElements=xmap%size, rc=rc)
   if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_MeshGet when getting xmap%size', FATAL)

   ! Write xgrid mesh for debugging
   !call ESMF_MeshWrite(xmesh, filename="xgridmesh", rc=rc)
   !if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at writing mesh from XGrid', FATAL)


   ! Create merge frac fields
   do g = 1, size(grid_ids(:))
      dst_merge_frac_x2g(g)=ESMF_FieldCreate(xmap%grids(g)%esmfGrid, typekind=ESMF_TYPEKIND_R8, rc=rc) 
      if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at tst_field', FATAL) 
   enddo

   do g = 1, size(grid_ids(:))
      dst_frac_x2g(g)=ESMF_FieldCreate(xmap%grids(g)%esmfGrid, typekind=ESMF_TYPEKIND_R8, rc=rc) 
      if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at tst_field', FATAL) 
   enddo

   esmfXgridField=ESMF_FieldCreate(xmap%esmfXgrid, typekind=ESMF_TYPEKIND_R8, rc=rc)
   if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldCreate for grid%esmfXgrid', FATAL)
...
!  Loops over grids
   ...
   do g = 1, 1
      ...
      ! Make fields necessary for regridding
      src_frac_g2x=ESMF_FieldCreate(xmap%grids(g)%esmfgrid, typekind=ESMF_TYPEKIND_R8, rc=rc)
      if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldCreate for grid%esmfXgrid', FATAL)

      call ESMF_FieldRegridStore(xmap%esmfXgrid, esmfField(g), esmfXgridField, &
                                 routehandle=xmap%grids(g)%g2x, &
                                 srcFracField=src_frac_g2x, &
                                 rc=rc)
      if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldRegridStore for grid to xgrid', FATAL)

#ifndef BOB_XGRID_DEBUG 
      call ESMF_FieldRegridStore(xmap%esmfXgrid, esmfXgridField, esmfField(g), &
                                 routehandle=xmap%grids(g)%x2g, &
                                 dstFracField=dst_frac_x2g(g), &
                                 dstMergeFracField=dst_merge_frac_x2g(g), &
                                 rc=rc)
      if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldRegridStore for xgrid to grid', FATAL)
#endif

      ! Get rid of fields used in setting up regridding
      call ESMF_FieldDestroy(src_frac_g2x, rc=rc)
      if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldDestroy for src_frac_g2x', FATAL)

      call ESMF_FieldDestroy(esmfField(g), rc=rc)
        if(rc /= ESMF_SUCCESS) call error_mesg("setup_esmf_grid", "fail at ESMF_FieldDestroy esmfField", FATAL)


      !! Set grid area
      ! Create Area Field wrapping grid%area
      areaField=ESMF_FieldCreate(xmap%grids(g)%esmfGrid, staggerloc=ESMF_STAGGERLOC_CENTER, &
           typekind=ESMF_TYPEKIND_R8, rc=rc)
      if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldCreate() for area field', FATAL)   

      ! Get ESMF Grid Area
      call ESMF_FieldRegridGetArea(areaField,rc=rc)
      if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldRegridGetArea() for area field', FATAL)   

      ! Get area pointer
      call ESMF_FieldGet(areaField,farrayPtr=dataPtr_area, rc=rc)
      if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for areaField', FATAL)

      ! Get Merge frac
      call ESMF_FieldGet(dst_merge_frac_x2g(g),farrayPtr=dataPtr_merge_frac, rc=rc)
      if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields(1)', FATAL)


      ! Set area and incorporate merge fraction, so area only counts part in xgrid
      grid => xmap%grids(g)
      do i1=lbound(dataPtr_area,1), ubound(dataPtr_area,1)
         do i2=lbound(dataPtr_area,2), ubound(dataPtr_area,2)
            grid%area(i1,i2)=dataPtr_area(i1,i2)*dataPtr_merge_frac(i1,i2)/sph_area  ! Divide by area of sphere, to match GFDL
         enddo
      enddo
   ...
   call ESMF_FieldDestroy(esmfXgridField, rc=rc)
   if(rc /= ESMF_SUCCESS) call error_mesg("setup_esmf_grid", "fail at ESMF_FieldDestroy xgridField", FATAL)


   !! DONE WITH SETTING UP XGRID, THE BELOW IS JUST DEBUGGING !!


   !! Check XGrid regridding as ESMF understands it. !!
#ifdef ESMF_XGRID_CHECK 

   ! ONLY DO IF ATM to XGrid to LND+OCN coupling
   if (size(grid_ids) .eq. 3) then
      if ((grid_ids(1) .eq. 'ATM') .and. &
          (grid_ids(2) .eq. 'OCN') .and. &
          (grid_ids(3) .eq. 'LND')) then

         ! Create test fields
         do g = 1, size(grid_ids(:))
            tst_fields(g)=ESMF_FieldCreate(xmap%grids(g)%esmfGrid, typekind=ESMF_TYPEKIND_R8, rc=rc) 
            if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at tst_field', FATAL) 
         enddo

         ! Create test Xgrid field
         tst_field_XGrid=ESMF_FieldCreate(xmap%esmfXGrid, typekind=ESMF_TYPEKIND_R8, rc=rc) 
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at tst_field_XGrid2', FATAL) 


         ! Create test Xgrid field
         tst_field_XGrid2=ESMF_FieldCreate(xmap%esmfXGrid, typekind=ESMF_TYPEKIND_R8, rc=rc) 
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at tst_field_Xgrid2', FATAL) 


         ! Set ATM Field
         call ESMF_FieldGet(tst_fields(1),farrayPtr=dataPtr_tst, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields(1)', FATAL)

         dataPtr_tst=1.0

         ! Set OCN Field
         call ESMF_FieldGet(tst_fields(2),farrayPtr=dataPtr_tst, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields(2)', FATAL)

         dataPtr_tst=0.0

         ! Set LND Field
         call ESMF_FieldGet(tst_fields(3),farrayPtr=dataPtr_tst, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields(3)', FATAL)

         dataPtr_tst=0.0

         ! Set Xgrid Field
         call ESMF_FieldGet(tst_field_Xgrid,farrayPtr=dataPtr_tst_XGrid, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields_XGrid', FATAL)

         dataPtr_tst_XGrid=0.0


         ! Regrid from ATM to XGrid
         call ESMF_FieldRegrid(tst_fields(1),tst_field_Xgrid, xmap%grids(1)%g2x, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldRegrid for tst_fields(1) to Xgrid', FATAL)   


         ! Calculate ATM conservation

         !! Create Area Field wrapping grid%area
         areaField=ESMF_FieldCreate(xmap%grids(1)%esmfGrid, staggerloc=ESMF_STAGGERLOC_CENTER, &
              typekind=ESMF_TYPEKIND_R8, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldCreate() for area field', FATAL)   
         ...
         l_sum(1)=0.0
         l_sum(2)=0.0
         do i1=lbound(dataPtr_tst,1), ubound(dataPtr_tst,1)
            do i2=lbound(dataPtr_tst,2), ubound(dataPtr_tst,2)
               l_sum(1)=l_sum(1) + dataPtr_area(i1,i2)*dataPtr_tst(i1,i2)
               l_sum(2)=l_sum(2) + dataPtr_area(i1,i2)
            enddo
         enddo

         call ESMF_VMAllReduce(vm, l_sum, g_sum, 2, ESMF_REDUCE_SUM, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at VMReduce()', FATAL)      

         atm_sum=g_sum(1)
         atm_area=g_sum(2)
         atm_norm_sum=atm_sum/atm_area
         if (localPet .eq. 0) then
            write(*,*) 
            write(*,*) " ESMF ATM sum=",atm_sum
            write(*,*) " ESMF ATM area=",atm_area
            write(*,*) " ESMF ATM norm sum=",atm_norm_sum
         endif

         ! Get rid of area Field
         call ESMF_FieldDestroy(areaField, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldDestroy() for area field', FATAL)      


         ! Calculate XGrid conservation

         !! Create Area Field wrapping xgrid
         areaField=ESMF_FieldCreate(xmesh, meshloc=ESMF_MESHLOC_ELEMENT, &
              typekind=ESMF_TYPEKIND_R8, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldCreate() for xgrid area field', FATAL)   
         ...
         !! Get area
         call ESMF_FieldGet(areaField,farrayPtr=dataPtr_area_XGrid, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for areaField', FATAL)
         ...
         l_sum(1)=0.0
         l_sum(2)=0.0
         do i1=lbound(dataPtr_tst_XGrid,1), ubound(dataPtr_tst_XGrid,1)
            l_sum(1)=l_sum(1) + dataPtr_area_XGrid(i1)*dataPtr_tst_XGrid(i1)
            l_sum(2)=l_sum(2) + dataPtr_area_XGrid(i1)
         enddo
         ...
         xgrid_sum=g_sum(1)
         xgrid_area=g_sum(2)
         xgrid_norm_sum=xgrid_sum/xgrid_area
         if (localPet .eq. 0) then
            write(*,*) 
            write(*,*) " ESMF XGRID sum=",xgrid_sum
            write(*,*) " ESMF XGRID area=",xgrid_area
            write(*,*) " ESMF XGRID norm sum=",xgrid_norm_sum
            write(*,('(a,ES)')) " ESMF Rel. Diff. A vs. X =",abs(atm_sum-xgrid_sum)/atm_sum
            write(*,('(a,ES)')) " ESMF Rel. Diff. Norm A vs. X =",abs(atm_norm_sum-xgrid_norm_sum)/atm_norm_sum
         endif

#if 0
         min_frac=minval(dataPtr_tst_XGrid)
         max_frac=maxval(dataPtr_tst_XGrid)

         write(*,*) " XGrid tst field values=",min_frac,max_frac
#endif
...
#ifndef BOB_XGRID_DEBUG
         ! Regrid from XGrid to OCN
         call ESMF_FieldRegrid(tst_field_Xgrid, tst_fields(2), xmap%grids(2)%x2g, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldRegrid for Xgrid to tst_fields(2)', FATAL)   


         ! Regrid from XGrid to LND
         call ESMF_FieldRegrid(tst_field_Xgrid, tst_fields(3), xmap%grids(3)%x2g, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldRegrid for Xgrid to tst_fields(3)', FATAL)   

         ! Calculate OCN conservation

         !! Create Area Field wrapping grid%area
         areaField=ESMF_FieldCreate(xmap%grids(2)%esmfGrid, staggerloc=ESMF_STAGGERLOC_CENTER, &
              typekind=ESMF_TYPEKIND_R8, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldCreate() for area field', FATAL)   
         ...
         ! Get OCN Merge frac
         call ESMF_FieldGet(dst_merge_frac_x2g(2),farrayPtr=dataPtr_merge_frac, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields(1)', FATAL)

         ! Get OCN frac
         call ESMF_FieldGet(dst_frac_x2g(2),farrayPtr=dataPtr_frac, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields(1)', FATAL)
         ...
         do i1=lbound(dataPtr_tst,1), ubound(dataPtr_tst,1)
            do i2=lbound(dataPtr_tst,2), ubound(dataPtr_tst,2)
               l_sum(1)=l_sum(1) + dataPtr_area(i1,i2)*dataPtr_tst(i1,i2)*dataPtr_merge_frac(i1,i2)
               l_sum(2)=l_sum(2) + dataPtr_area(i1,i2)*dataPtr_merge_frac(i1,i2)
            enddo
         enddo
         ...
         ! Calculate LND conservation

         !! Create Area Field wrapping grid%area
         areaField=ESMF_FieldCreate(xmap%grids(3)%esmfGrid, staggerloc=ESMF_STAGGERLOC_CENTER, &
              typekind=ESMF_TYPEKIND_R8, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldCreate() for area field', FATAL)   
         ...
         ! Get LND Merge frac
         call ESMF_FieldGet(dst_merge_frac_x2g(3),farrayPtr=dataPtr_merge_frac, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields(1)', FATAL)

         ! Get LND frac
         call ESMF_FieldGet(dst_frac_x2g(3),farrayPtr=dataPtr_frac, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields(1)', FATAL)
         ...
         ! Sum 
         ! DON'T INIT BECAUSE ADDING TO OCN SUM   l_sum(1)=0.0
         do i1=lbound(dataPtr_tst,1), ubound(dataPtr_tst,1)
            do i2=lbound(dataPtr_tst,2), ubound(dataPtr_tst,2)
               l_sum(1)=l_sum(1) + dataPtr_area(i1,i2)*dataPtr_tst(i1,i2)*dataPtr_merge_frac(i1,i2)
               l_sum(2)=l_sum(2) + dataPtr_area(i1,i2)*dataPtr_merge_frac(i1,i2)
         ...
         lo_sum=g_sum(1)
         lo_area=g_sum(2)
         lo_norm_sum=lo_sum/lo_area
         if (localPet .eq. 0) then
            write(*,*)   
            write(*,*) " ESMF L+O sum=",lo_sum
            write(*,*) " ESMF L+O area=",lo_area
            write(*,*) " ESMF L+O norm sum=",lo_norm_sum
            write(*,('(a,ES)')) " ESMF Rel. Diff. A vs. LO =",abs(atm_sum-lo_sum)/atm_sum
            write(*,('(a,ES)')) " ESMF Rel. Diff. Norm A vs. LO =",abs(atm_norm_sum-lo_norm_sum)/atm_norm_sum
            write(*,*)   
            write(*,*)   
            write(*,*)   
         endif

        !! Regrid backwards from land to ATM to check for bad land points !!
#if 0 
         ! Set ATM Field
         call ESMF_FieldGet(tst_fields(1),farrayPtr=dataPtr_tst, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields(1)', FATAL)
         ...
         ! Set to 10.0 where unmasked
         do i1=lbound(dataPtr_tst,1), ubound(dataPtr_tst,1)
            do i2=lbound(dataPtr_tst,2), ubound(dataPtr_tst,2)
               if (dataPtr_mask(i1,i2) .ne. 1) then
                  dataPtr_tst(i1,i2)=10.0
               endif
            enddo
         enddo
         ...
         dataPtr_tst=1.0 ! Init to 1.0 so we can see where it ends up in the ATM
         ...
         ! Set Xgrid Field
         call ESMF_FieldGet(tst_field_Xgrid2,farrayPtr=dataPtr_tst_XGrid2, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldGet for tst_fields_XGrid', FATAL)

         dataPtr_tst_XGrid2=0.0


         ! Regrid from LND to XGrid
         call ESMF_FieldRegrid(tst_fields(3),tst_field_Xgrid, xmap%grids(3)%g2x, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldRegrid for tst_fields(1) to Xgrid', FATAL)   


         ! Regrid from OCN to XGrid2
         call ESMF_FieldRegrid(tst_fields(2),tst_field_Xgrid2, xmap%grids(2)%g2x, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldRegrid for tst_fields(1) to Xgrid', FATAL)   

         ! Get distgrid
         call ESMF_XGridGet(xmap%esmfXGrid, distgridM=tmpDistgrid, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at XGridGet distgrid', FATAL)   

         ! Get sequence ids
         allocate(seqInd(size(dataPtr_tst_XGrid,1)))
         call ESMF_DistgridGet(tmpDistgrid, localDe=0, seqIndexList=seqInd, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at DistgridGet SeqInd', FATAL)   


         ! Loop looking for land in ocean
         do i1=lbound(dataPtr_tst_XGrid,1), ubound(dataPtr_tst_XGrid,1)
            if ((dataPtr_tst_XGrid(i1) .gt. 0.0) .and. &
                 (dataPtr_tst_XGrid2(i1) .gt. 0.0)) then
               i2=i1-lbound(dataPtr_tst_XGrid,1)+1
               write(*,*) " LND IN OCN id=",seqInd(i2),"l=",dataPtr_tst_XGrid(i1),"o=",dataPtr_tst_XGrid2(i1)
            endif
         enddo

         ! Deallocate sequence ids
         deallocate(seqInd)
#endif

         !! Regrid constant 1 field from L+O to XGrid and then to ATM !!
         ...
         ! Regrid from OCN to XGrid
         call ESMF_FieldRegrid(tst_fields(2),tst_field_Xgrid, xmap%grids(2)%g2x, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldRegrid for tst_fields(1) to Xgrid', FATAL)   


         ! Regrid from LND to XGrid
         call ESMF_FieldRegrid(tst_fields(3),tst_field_Xgrid, xmap%grids(3)%g2x, &
         ...
         ! Regrid from XGrid to ATM
         call ESMF_FieldRegrid(tst_field_Xgrid, tst_fields(1), xmap%grids(1)%x2g, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldRegrid for Xgrid to tst_fields(3)', FATAL)   
         ...
         do i1=lbound(dataPtr_tst,1), ubound(dataPtr_tst,1)
            do i2=lbound(dataPtr_tst,2), ubound(dataPtr_tst,2)
               grid%area(i1,i2)=dataPtr_area(i1,i2)/sph_area  ! Divide by area of sphere, to match GFDL
            enddo
         enddo
#endif
         ...
         datm_sum=g_sum(1)
         datm_area=g_sum(2)
         datm_norm_sum=datm_sum/datm_area
         if (localPet .eq. 0) then
            write(*,*) 
            write(*,*) " ESMF dest ATM sum=",datm_sum
            write(*,*) " ESMF dest ATM area=",datm_area
            write(*,*) " ESMF dest ATM norm sum=",datm_norm_sum
            write(*,('(a,ES)')) " ESMF Rel. Diff. d A vs. A =",abs(atm_sum-datm_sum)/atm_sum
            write(*,('(a,ES)')) " ESMF Rel. Diff. Norm d A vs. A =",abs(atm_norm_sum-datm_norm_sum)/atm_norm_sum
            write(*,*)   
            write(*,*)   
            write(*,*)   
         endif
         ...
         ! Get rid of tst Fields
         do g = 1, size(grid_ids(:))
            call ESMF_FieldDestroy(tst_fields(g), rc=rc)
            if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldDestroy(tst_field)', FATAL) 
         enddo

         call ESMF_FieldDestroy(tst_field_Xgrid, rc=rc)
         if(rc /= ESMF_SUCCESS) call error_mesg('xgrid_mod', 'Failed at ESMF_FieldDestroy(tst_field_Xgrid)', FATAL) 

#else
         ! Stop if we are in debug mode
         stop
#endif

      endif ! If is ATM to XGrid to LND+OCN coupling
   endif ! If there are 3 grids

  !! End check ESMF XGrid regridding !!
#endif

end subroutine setup_esmf_grid

subroutine check_coord_on_pet(grid, petno, rc)
